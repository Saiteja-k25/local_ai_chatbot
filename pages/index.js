  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        const formatted = data.reverse().flatMap(chat => ([
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      ...
      <div className="max-w-2xl mx-auto flex flex-col h-[90vh]">
        <h1 className="text-3xl font-bold mb-4 text-center">💬 Local AI Chatbot (phi)</h1>
        ...
        <div className="flex-1 overflow-y-auto space-y-4 mb-4 scrollbar-thin scrollbar-thumb-gray-700">
          {messages.map((msg, idx) => (
            <motion.div
              ...
              className={`p-3 rounded-xl max-w-[80%] whitespace-pre-wrap break-words ${msg.role === 'user' ? 'bg-blue-600 ml-auto' : 'bg-gray-700'}`}
            >
              {msg.content}
            </motion.div>
          ))}
